...
import { FixAction } from './llmProvider';
import { BuiltContext, FileContext } from './contextBuilder';
...
export class AnalysisWebview {
  ...
  private currentActions: FixAction[] = [];
  ...
  showActions(actions: FixAction[]): void {
  ...
  private updateContent(): void {
    ...
    this.panel.webview.html = this.getHtmlTemplate(
      categoryHtml, terminalHtml, codeContextHtml, stackHtml, analysisHtml, actionsHtml
    );
  }
  ...
  private getHtmlTemplate(categoryHtml: string, terminalHtml: string, codeContextHtml: string, stackHtml: string, analysisHtml: string, actionsHtml: string): string {
    return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
