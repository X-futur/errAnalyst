...
- 色板取自 VS Code 自带主题引擎实测值（Dark+ / Light+），与编辑器默认 Python 高亮基本一致；Prism 与 MagicPython 归类差异处（如 `@staticmethod` 装饰器名、raw 字符串、f-string 占位符、`self` 参数位）按最近似类别近似，不做像素级复刻。
